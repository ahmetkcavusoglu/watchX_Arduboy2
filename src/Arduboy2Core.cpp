...
// these commands are only supported by SSD1306 and SSD1309
...
{
PORTB |= _BV(LEFT_BUTTON_BIT) | _BV(RIGHT_BUTTON_BIT) | _BV(A_BUTTON_BIT) ;
PORTB &= ~(_BV(SPEAKER_1_BIT));
DDRB &= ~(_BV(LEFT_BUTTON_BIT) | _BV(RIGHT_BUTTON_BIT) | _BV(A_BUTTON_BIT) |
            _BV(SPI_MISO_BIT));
 ...
 DDRD &= ~(_BV(DOWN_BUTTON_BIT) | _BV(B_BUTTON_BIT) | _BV(RAND_SEED_IN_BIT));
  PORTD &= ~(_BV(LEDR_BIT) | _BV(RAND_SEED_IN_BIT));
 DDRD |= _BV(LEDR_BIT);
...
buttons = ((~PINB) &
              (_BV(A_BUTTON_BIT) | _BV(RIGHT_BUTTON_BIT) |
               _BV(LEFT_BUTTON_BIT)));
  // A
